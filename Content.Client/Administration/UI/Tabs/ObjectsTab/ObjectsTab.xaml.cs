...
public sealed partial class ObjectsTab : Control
{
    ...
    private void RefreshObjectList(ObjectsTabSelection selection)
    {
        ...
        var (altColor, defaultColor) = GetListColors();
        ...
        for (var index = 0; index < entities.Count; index++)
        {
            ...
            listData.Add(new ObjectsListData(info,
                ...
                index % 2 == 0 ? altColor : defaultColor));
        }

        SearchList.PopulateList(listData);
    }

    private (Color Alt, Color Default) GetListColors()
    {
        var alt = UiThemeHelper.AccentTint(_cfg, "#252733F0", 0.26f);
        var normal = UiThemeHelper.AccentTint(_cfg, "#2D3040F5", 0.33f);
        return (alt, normal);
    }
