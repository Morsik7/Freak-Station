...
using Content.Client._OS.UI.AnimatedBackground;
...
namespace Content.Client.Lobby
{
    public sealed class LobbyState : Robust.Client.State.State
    {
        ...
        private void UpdateLobbyBackground()
        {
            ...
            {
                var backgroundId = _gameTicker.LobbyBackground.Value;

                if (_protoMan.TryIndex(backgroundId, out LobbyBackgroundPrototype? lobbyBackground))
                {
                    // Vanilla path: lobby background from prototype.
                    if (_resourceCache.TryGetResource(lobbyBackground.Background, out TextureResource? protoTexture))
                    {
                        Lobby!.Background.Texture = protoTexture.Texture;
                        return;
                    }

                    if (Lobby!.Background is AnimatedBackgroundControl animated
                        && _resourceCache.TryGetResource(lobbyBackground.Background, out RSIResource? protoRsi))
                    {
                        animated.SetRSI(protoRsi.RSI);
                        return;
                    }
                ...
                var path = new ResPath(backgroundId.Id);

                // Compatibility fallback: some forks send a direct texture path instead of prototype ID.
                if (_resourceCache.TryGetResource(path, out TextureResource? texture))
                {
                    Lobby!.Background.Texture = texture.Texture;
                    return;
                }

                // OpenSpace fallback: direct RSI path for animated lobby backgrounds.
                if (Lobby!.Background is AnimatedBackgroundControl directAnimated
                    && _resourceCache.TryGetResource(path, out RSIResource? directRsi))
                {
                    directAnimated.SetRSI(directRsi.RSI);
                    return;
                }

                _sawmill.Warning($"Unknown lobby background id '{backgroundId.Id}' - expected lobbyBackground prototype or texture path.");
                Lobby!.Background.Texture = null;
